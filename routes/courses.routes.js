...
router.get("/:id", userLogged, idFormat, (req, res) => {
    ...
    Course
        ...
        .then(course => {
            ...
            res.render('courses/course-details', { course, isTA })
        })
        .catch(err => console.error(err));    
});
    ...
    Course
        ...
        .then(course => {
            ...
            if(studIds.includes(req.session.currentUser._id)) {
                res.render('courses/course-details', { course, errorMsg: 'You are already enroled in this course' });
                return;
            } else students.push(req.session.currentUser._id);

            Course
                .findByIdAndUpdate(req.params.id, { students })
                .then(() => res.redirect('/courses/' + req.params.id))
                .catch(err => console.log(err));
        })
})
